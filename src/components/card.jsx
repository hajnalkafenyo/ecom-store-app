...
export function Card({ id, title, description, image, price, discountedPrice, rate }) {
    const navigate = useNavigate()
    return (
        <div className="border border-gray-300 w-[200px] rounded p-2 flex flex-col bg-white hover:bg-gray-50 hover:border-amber-400 transition-colors cursor-pointer" role="button" tabIndex={0} onClick={() => {
            navigate(`/product/${id}`)
        }}>
            <div className="flex flex-col object-fit">
                <img src={image} className="h-[160px] w-full object-cover" alt="My awesome Product" />
            </div>
